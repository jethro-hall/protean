...
| File | Role |
|---|---|
| `protean-design-system.css` | **The design specification** (per ADR-0004 — *not* the app's stylesheet). Tokens + base + every component, as a rich reference the live app is grown *toward*. Only the static prototype/style-guide pages link it; **no React component imports it.** |
...
**Superseded by ADR-0004 (2026-07-28).** The original plan was to bulk-promote this CSS into
`APP/GUI/src/theme/` at Phase 1. That is **no longer the approach.** The single source of visual
truth is `APP/GUI/src/theme/tokens.css` (Tailwind v4 `@theme`), which the live GUI already consumes.
This file's vocabulary — semantic role aliases, the purple reasoning role, spacing/radius/shadow/
z/motion/type scales — is ported into `tokens.css` **on demand**, one token at a time as a shipping
component actually consumes it, **never as a bulk paste**. See `docs/DECISIONS/ADR-0004-design-system-source-of-truth.md`.
