...
/** Gradient identity chip beside each message — assistant = blue gradient, user = ink (prototype .who-ico). */
function Avatar({ isUser }: { isUser: boolean }) {
  return (
    <span
      aria-hidden
      className={`mt-0.5 flex h-7 w-7 shrink-0 select-none items-center justify-center rounded-lg text-[11px] font-semibold text-surface shadow-sm ${
        isUser ? 'bg-ink' : 'bg-gradient-to-br from-accent-blue to-accent-blue-2'
      }`}
    >
      {isUser ? 'You' : 'P'}
    </span>
  ...
  return (
    <div className={`flex gap-2.5 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
      ...
      <div
        className={
          isUser
            ? 'max-w-[85%] rounded-2xl rounded-tr-sm bg-accent-blue px-4 py-2.5 text-sm text-surface'
            : 'max-w-[85%] rounded-2xl rounded-tl-sm border border-line bg-surface px-4 py-2.5 text-sm shadow-sm'
        }
      >
    ...
    return (
      <div className="flex flex-1 items-center justify-center p-8 text-center">
        <div className="flex flex-col items-center">
          <span
            aria-hidden
            className="mb-3 h-12 w-12 rounded-xl bg-gradient-to-br from-accent-blue to-accent-blue-2 shadow-md ring-1 ring-inset ring-surface/40"
          />
